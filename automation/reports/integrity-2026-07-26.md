...
> Previous audit: integrity-2026-07-19.md (13 contradictions across 5 books).
> **Net change this week: 0 new contradictions** — all 13 prior contradictions remain unresolved.
>
...
> - `fix-your-gut-for-good` KDP Select term **expired 2026-07-19** (7 days ago). `publishing.kdp_select_term_end = "2026-07-19"`. The pipeline state does not reflect re-enrolment. Next Countdown Deal window: `post_launch.countdown_deal_eligible_next_term = "2026-08-18"` — 23 days away.
> - `vagus-nerve-gut-reset-workbook` has been **cleared for upload since 2026-06-26** (30 days ago) with all Stage 06 gates passed (`final_approval_passed = true`, `cover_approved = true`). Still `published = false`, `kdp_status = "not_started"`. No upload agent has been invoked.
> - `h-pylori-recovery-plan` has been **live on Amazon for 39 days** (since 2026-06-17) with `stages["10-postlaunch"].status = "not_started"` and zero post-launch data logged.
...
CONTRADICTION *(unresolved — 3rd consecutive week)*: `production.pdf_built = true` and `production.docx_built = true`, but the production block contains no `pdf_file`, `pdf_path`, `docx_file`, or `docx_path` field — these fields are entirely absent. Only `epub_file` and `cover_file` are recorded.
...
CONTRADICTION *(unresolved — 7th consecutive week, first detected 2026-06-14)*: `quality_scores.final_approval_score = null` but `human_gates.final_approval_passed = true`. The pipeline requires a minimum 270/300 from `final-approval-agent` before this gate can be set true. With a null score the 270/300 threshold cannot be verified. No `FINAL-APPROVAL.md` exists on disk (confirmed by prior audits).
...
CONTRADICTION *(unresolved — 3rd consecutive week)*: `last_updated = 2026-05-28T00:00:00Z` but the newest `agent_log` entry has timestamp `2026-06-02T00:00:00Z`. Two agent_log entries (2026-06-01 and 2026-06-02) were appended without updating `last_updated` — a 5-day gap.
...
### Check 2 — current_stage consistency
CONTRADICTION *(unresolved — 3rd consecutive week)*: `current_stage = 10` (`10-postlaunch` is `"in_progress"`) but `stages["08-products"].status = "in_progress"` and `stages["09-series"].status = "pending"`. Stage 10 is open while stages 08 and 09 are themselves incomplete, with no explanation for the skip.
...
### Check 5 — production.*_built vs file paths
CONTRADICTION *(unresolved — 3rd consecutive week)*: `production.pdf_built = true` but `production.pdf_path = null`; `production.docx_built = true` but `production.docx_path = null`. The build note explicitly acknowledges: "Export files present in exports/final/ — exact paths not verified in this session."
...
**Pipeline version:** 2.0 | **Current stage:** 9 | **Last updated:** 2026-07-06 | **KDP status:** live (ASIN B0H5TZTPRT, live 2026-06-17 — **39 days ago as of today**)

### Check 1 — human_gates.published vs kdp_status
PASS — `human_gates.published = true` and `publishing.kdp_status = "live"`. Consistent.

### Check 2 — current_stage consistency
CONTRADICTION *(unresolved — 3rd consecutive week)*: The book has been live on Amazon for 39 days (`live_date = "2026-06-17"`) yet `current_stage = 9` and `stages["10-postlaunch"].status = "not_started"`. All stages 00–09 are `"complete"`. No post-launch monitoring, BSR tracking, ARC follow-up, or ad-gate check has been initiated. Every `post_launch` counter sits at zero with no logged dates. `last_updated = 2026-07-06` — 20 days of additional silence since the last state touch.
...
### Check 3 — Date chronology
CONTRADICTION *(unresolved — 2nd consecutive week)*: `stages["05-optimisation"].started_at = "2026-06-11T11:00:00Z"` and `stages["05-optimisation"].completed_at = "2026-06-11T12:00:00Z"`, but `stages["04-quality"].completed_at = "2026-06-11T13:00:00Z"`. Stage 05 (optimisation) both started and completed before Stage 04 (quality) closed — the sequential gate rule was violated in the recorded state.
→ Fields involved: `stages["05-optimisation"].started_at = "2026-06-11T11:00:00Z"` and `.completed_at = "2026-06-11T12:00:00Z"` vs `stages["04-quality"].completed_at = "2026-06-11T13:00:00Z"`
→ Suggested fix: Verify with session transcripts whether Stage 04 genuinely completed before Stage 05 began. Correct `stages["04-quality"].completed_at` to the true chronology, or add a reconciliation note acknowledging the sequence deviation.

### Check 4 — Gap flag vs resolved note
CONTRADICTION *(unresolved — 3rd consecutive week)*: `pre_launch.launch_ready = false` and `pre_launch.open_items` still contains the active item "Recruit ARC cohort to ≥20 commitments", yet `human_gates.published = true`. The pre-launch gate was never formally closed before publication.
→ Fields involved: `pre_launch.launch_ready = false` vs `human_gates.published = true`
→ Suggested fix: Decide whether to retroactively close the pre-launch gate with a reconciliation note, or leave it false with documentation that the book launched outside the formal process. ARC recruitment remains actionable post-launch for review velocity.
...
→ Suggested fix: Run `final-approval-agent` against the full manuscript package. Record the 300-point score in a dedicated field. If ≥270, final_approval_passed is verified; if <270, remediation required before this gate can stand.

### Check 7 — last_updated vs newest agent_log timestamp
PASS — `last_updated = 2026-07-06T00:00:00Z` matches the newest `agent_log` entry `2026-07-06T00:00:00Z`. Consistent.

**Contradictions in this book: 4** (unchanged from prior audit)
...
### Check 2 — current_stage consistency
CONTRADICTION *(unresolved — 7th consecutive week, first detected 2026-06-14)*: `writing.completed_chapters = 1` and `writing.approved_chapters = 1`, but `writing.current_chapter = "04-chapter-4"`. The `agent_log` (2026-04-29) explicitly states: "Chapter 3 complete and approved. Resuming from Chapter 4." At minimum 3 chapters should be recorded as complete. The state records only 1. Chapters 1 and 2 are absent from `manuscript/` on disk; only `03-chapter-3.md` exists. This novel cannot be assembled without them.
...
**Pipeline version:** (not set) | **Current stage:** 9 | **Last updated:** 2026-06-13 | **Lifecycle:** parked | **KDP status:** null — not published (planned launch 2026-05-29, now 58 days past without publication)
...
### Check 3 — Date chronology
CONTRADICTION *(unresolved — 3rd consecutive week)*: `agent_log` entry at array index 5 has `timestamp = "2026-05-23T12:00:00Z"` but appears in the array after three entries timestamped 2026-05-24 (09:00, 14:00, 17:00). The 2026-05-23 entry is at least 9 hours out of sequence relative to its neighbours.
...
### Check 5 — production.*_built vs file paths
CONTRADICTION *(unresolved — 3rd consecutive week)*: `production.cover_built = true` but `production.build_note` contains the text "cover_built=false — blocked on human cover generation per COVER-BRIEF.md (sole remaining blocker)." This note is stale — written during the 2026-05-21 build before the cover was generated — and was never updated when the cover was subsequently built and `cover_built` set to `true`.
...
**Pipeline version:** 2.0 | **Current stage:** 6 | **Last updated:** 2026-06-26T21:30 | **KDP status:** not started (cleared for upload — all Stage 06 gates passed; 30 days without upload action)
...
### Check 7 — last_updated vs newest agent_log timestamp
CONTRADICTION *(unresolved — 3rd consecutive week)*: `last_updated = 2026-06-26T21:30:00Z` but the newest `agent_log` entry has `timestamp = "2026-06-28T18:05:00Z"`. Two build/hygiene entries from 2026-06-28 (17:38 and 18:05) were appended without refreshing `last_updated` — a gap of approximately 44 hours.
...
The directory `books/cortisol-gut-health/` continues to exist with only `RUN-LEDGER.md` and **no `pipeline-state.json`** — same as reported in every prior audit. This book remains invisible to any pipeline agent that reads state. If active, a `pipeline-state.json` must be created from `pipeline-state.template.json` before any pipeline work begins.
...
|------|--------------|----------------|:----------:|
| death-in-the-cathedral-close | final_approval_score=null / final_approval_passed=true / no FINAL-APPROVAL.md | 2026-06-14 | **7** |
| the-dust-between-seconds | completed_chapters=1 vs current_chapter=04-chapter-4 / chapters 1–2 missing from disk | 2026-06-14 | **7** |
| death-in-the-cathedral-close | pdf_built=true + docx_built=true but no pdf_file/docx_file path fields | 2026-07-13 | 2 |
| death-in-the-cathedral-close | last_updated=2026-05-28 < newest agent_log=2026-06-02 | 2026-07-13 | 2 |
| fix-your-gut-for-good | current_stage=10 but stages 08 (in_progress) and 09 (pending) incomplete | 2026-07-13 | 2 |
| fix-your-gut-for-good | pdf_built=true/pdf_path=null; docx_built=true/docx_path=null | 2026-07-13 | 2 |
| h-pylori-recovery-plan | book live since 2026-06-17 but current_stage=9 / stage 10 not_started | 2026-07-13 | 2 |
| h-pylori-recovery-plan | pre_launch.launch_ready=false but human_gates.published=true | 2026-07-13 | 2 |
| h-pylori-recovery-plan | final_approval_passed=true but final_approval_score=113 (book-reviewer /120, not 300-pt audit) | 2026-07-13 | 2 |
| h-pylori-recovery-plan | stage 05 started_at (11:00) and completed_at (12:00) both predate stage 04 completed_at (13:00) | 2026-07-19 | 1 |
| the-7-day-gut-reset | agent_log entry 2026-05-23T12:00 appears after three 2026-05-24 entries | 2026-07-13 | 2 |
| the-7-day-gut-reset | production.build_note says "cover_built=false" but production.cover_built=true | 2026-07-13 | 2 |
| vagus-nerve-gut-reset-workbook | last_updated=2026-06-26T21:30 < newest agent_log=2026-06-28T18:05 | 2026-07-13 | 2 |
...
**Check 6 — final_approval_score=113 (< 270/300 threshold) vs final_approval_passed=true (compounded: 4 total contradictions, live book, 39 days without post-launch tracking)**

`quality_scores.final_approval_score = 113` is the `book-reviewer` score on a /120 scale, not a 300-point `final-approval-agent` output. The pipeline's 270/300 gate was never satisfied by a proper audit — `final_approval_passed` was set via Architect override (cover gate only) without a complete 300-point run. The `lifecycle_note` explicitly flags this as "INV-9 final_approval_score 113<270." This book is **live on Amazon** (39 days, ASIN B0H5TZTPRT) with an unverified quality gate, a never-started Stage 10, and zero post-launch data recorded. Combined with the stage 04/05 sequence violation (Check 3), the launch_ready=false contradiction (Check 4), and the Check 2 post-launch gap, this book carries 4 contradictions — the most of any book in the portfolio and the most operationally consequential.

**Recommended immediate actions:**
1. Run `post-launch-tracker` now with current KDP dashboard data (BSR, review count, KU pages) — 39 days of post-launch data has been missed; log at minimum a current baseline.
2. Advance `current_stage` to 10; set `stages["10-postlaunch"].status = "in_progress"` with `started_at = "2026-06-17"`.
3. Run `final-approval-agent` against the full manuscript package to produce a verified 300-point score; record it in `quality_scores.final_approval_score`.
4. Investigate the stage 04/05 timestamp overlap and correct `stages["04-quality"].completed_at` to reflect true chronology.
5. All updates via agent-log MODE 2.

---

*Audit run: 2026-07-26 | Audit type: automated weekly state integrity | Files checked: 6 pipeline-state.json + 1 missing (cortisol-gut-health) | Files modified: 0*
