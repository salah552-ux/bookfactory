# BookFactory Milestone Sentinel — 2026-06-13
## ACTION REQUIRED (5 items)

Generated: 2026-06-13 | All calculations use today's date.
...
Days until 2026-07-19: **36 days**. Threshold not met.
> Watch-list: decision window opens **2026-07-05** (14 days before end; 22 days from today). KDP Select auto-renews unless opted out — set a calendar reminder.
...
**ok** — File field `countdown_deal_eligible_from: null`; anchor says eligibility opened 2026-05-21 (23 days ago). Next window ~2026-08-18 (anchor). Days until: **66 days**. Threshold not met.
...
**FIRED** — Critical state integrity failure. Anchor records the book as live since **2026-04-21** (53 days ago). File shows:
...
The state file was never reconciled after KDP go-live. All post-launch tracking (KDP Select term, countdown eligibility, ASIN, ads) is unrecorded and untrackable. This has now been outstanding **53 days** — up from 52 days on 2026-06-12.
...
### Rule 1 — KDP Select term ends within 14 days
**ok** — `kdp_select: true`, `live_date: "2026-05-03"`. Computed term end (90 days from live date): **2026-08-01**. Days until: **49 days**. Threshold not met.
> Watch-list: re-enrollment decision needed by **2026-07-18** (14 days before computed end; 35 days from today).

### Rule 2 — Countdown Deal ends within 3 days
**ok** — No Countdown Deal currently active per file (`countdown_deal_run: false`).
> Advisory (unresolved since 2026-06-10): Anchor shows deal scheduled 2026-06-02 to 2026-06-09; that window ended **4 days ago**. File still shows `countdown_deal_run: false`. Rule 2 does not fire today (end date is past), but the discrepancy remains unresolved. Verify in KDP dashboard: if the deal ran, update `countdown_deal_run: true` via agent-log MODE 2. If it did not run, schedule the next deal — see Rule 3 advisory below.

### Rule 3 — Countdown-eligibility window opens within 7 days
**ok** — `countdown_deal_eligible_from: "2026-06-02"` (file). Eligibility window is already open (opened 11 days ago). Rule threshold not triggered (window already open, not opening within 7 days).
> Advisory: Window is **currently open** and `countdown_deal_run: false`. If the June deal did not run, a new deal can still be scheduled now (KDP requires 3 days' advance notice, max 7-day duration). The KDP Select term ends ~**2026-08-01** (49 days). Act before then — do not let the entire enrollment period expire without a promotion run. If the June deal did run, the next window opens ~**2026-07-09** (26 days away; 30 days after deal end per KDP rules).

### Rule 4 — bookfunnel_link_live == false
**FIRED** — Field absent from state file; anchor confirms `false`. BookFunnel lead-magnet link is not live for a book now **41 days post-launch**.
Standing GAP — repeats every sentinel run until resolved.
> **Action:** Activate BookFunnel link, then update state file with `bookfunnel_link_live: true` via agent-log MODE 2.
...
### Rule 6 — Stage in_progress > 14 days ago
**FIRED** — Stage `10-postlaunch` has been `in_progress` since **2026-05-03** (**41 days ago**). Threshold: 14 days. No progress updates recorded in 41 days. Key post-launch metrics all at zero:
- `ads_active: false` (41 days post-launch, no ads ever started)
...
|------|-------------|----------------|
| 1 — KDP Select ends <14 days | ok (36 days to 2026-07-19 anchor) | ok (49 days to 2026-08-01 computed) |
| 2 — Countdown Deal ends <3 days | ok (no deal) | ok — deal ended 4 days ago per anchor; advisory unresolved |
| 3 — Countdown eligibility opens <7 days | ok (next window ~2026-08-18, 66 days) | ok — window already open; deal unrun; see advisory |
| 4 — bookfunnel_link_live==false | **FIRED** — standing GAP | **FIRED** — standing GAP (41 days post-launch) |
| 5 — category_mismatch_flagged==true | ok | **FIRED** — unverified, needs KDP dashboard check |
| 6 — Stage in_progress >14 days | ok (no in_progress stage) | **FIRED** — 10-postlaunch stuck **41 days** |
| 7 — Human gate false on live book | **FIRED** — state file unreconciled (53 days) | ok |
| **Total fired** | **2** | **3** |

**Grand total: 5 rules fired across 2 books.** (Unchanged from 2026-06-12. Day counts incremented by 1.)

---

### Most Urgent Item
**Cathedral Close — Stage 10-postlaunch stuck 41 days with zero commercial activity**
`10-postlaunch` has been `in_progress` since 2026-05-03. No ads have ever run. Zero ARC emails sent. Zero reviews. Zero A+ content. At 41 days post-launch, the organic ranking window is closing fast. Every day without ads and reviews compounds rank decay.
...
- [ ] **Fix Your Gut:** State file unreconciled — book may be live 53 days with zero post-launch tracking
- [ ] **Cathedral Close:** Countdown Deal status unknown — verify KDP dashboard (deal window 2026-06-02–2026-06-09 per anchor); if deal did not run, schedule new one before 2026-08-01
- [ ] **Cathedral Close:** Category mismatch unverified — needs KDP dashboard check (open since 2026-06-10)
- [ ] **Cathedral Close:** Stage 10 stuck 41 days — no ads, no ARC, 0 reviews
...
|------|----------|---------------|-------------|-------------------|
| Fix Your Gut for Good | 2026-07-19 (anchor) | 36 days | **2026-07-05** | Opt out or confirm re-enroll by 2026-07-19 |
| Death in the Cathedral Close | 2026-08-01 (computed) | 49 days | **2026-07-18** | Opt out or confirm re-enroll by 2026-08-01 |
...
> Note: A third book (`the-dust-between-seconds`) was discovered in the books/ directory during this scan. Stage 03-writing has been `in_progress` since 2026-04-30 (44 days) — Rule 6 would fire for this book if it were in scope. Recommend adding it to future sentinel scans.
