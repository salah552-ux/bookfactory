# Review Velocity Report — 2026-08-07

**Total new reviews this week (both books combined): UNVERIFIABLE — Amazon fetch blocked (14th consecutive block). Prior values used. Both books remain at 0 verified reviews. Review-generation action required on both titles — both are stalled at Pre-Gate 1 (ads locked).**
...
Amazon's bot-protection/proxy layer continues to block all automated fetch attempts. This is the **14th consecutive blocked run** (first block observed 2026-06-07, 9+ weeks ago). **No review counts have been invented.** Per hard rules, prior values (0) are used and flagged as unverified.

---

## Review Velocity Table

| Book | ASIN | Prior Count (2026-07-31) | Current Count | New This Period | Days Elapsed | Reviews / Week | Current Gate | To Next Gate | ETA to Next Gate |
|------|------|--------------------------|---------------|----------------|-------------|---------------|-------------|-------------|-----------------|
...
- **Reviews to Gate 1:** 5
- **Days since live date:** 108 days (live since 2026-04-21)
- **Velocity:** 0 reviews/week (unverified; fetch blocked since 2026-06-07)
- **ETA to Gate 1:** STALLED — review-generation action needed
- **KDP Select status:** Term expired 2026-07-19 (19 days ago) — re-enrolment decision still outstanding. Next Countdown Deal window opens ~2026-08-18 only if re-enrolled. This date is now 11 days away — **urgent action required.**

### Death in the Cathedral Close (B0GZD1S8HF)

- **Current gate:** Pre-Gate 1 (0 / 5 reviews — unverified; fetch blocked)
- **Reviews to Gate 1:** 5
- **Days since live date:** 96 days (live since 2026-05-03)
- **Velocity:** 0 reviews/week (unverified; fetch blocked; 96 days live, 0 verified reviews)
- **ETA to Gate 1:** STALLED — review-generation action needed

---

## ⚠️ Critical Alert: KDP Select Term Expired (Fix Your Gut for Good) — 19 Days Overdue

`publishing.kdp_select_term_end` was **2026-07-19**, now 19 days past. The Countdown Deal window (~2026-08-18) is only 11 days away — if not re-enrolled **now**, the window will pass and the next opportunity defers another 90 days.

If not re-enrolled:
- KU page-read income has stopped
- The 2026-08-18 Countdown Deal window is lost
- Free promotion days in the new term unavailable

**Action required immediately:** Log into KDP dashboard and confirm Select re-enrolment status for B0GXYLWS1W.

---

## ⚠️ Escalation: 14-Week Stall on Reviews — Both Books

Both books have been live for 96–108 days with zero verified reviews and zero velocity. **Gate 1 (5 reviews) is the ads-unlock gate — Amazon Ads cannot be activated until 5 reviews are reached.** At current velocity, the pipeline is earning zero ad-driven income on two completed, published books.

### Priority 1 — ARC Reader Outreach (highest conversion, currently dead)

- `post_launch.arc_emails_sent = 0` for **both** books — 96–108 days post-launch with zero ARC contact
...
- Amazon.co.uk and Amazon.com have both been blocked by the network egress proxy for 14 consecutive weekly runs (9+ weeks)
...
books/fix-your-gut-for-good/pipeline-state.json
  post_launch.weekly_log → append:
    { "date": "2026-08-07", "review_count": 0, "avg_rating": null, "source": "fetch-blocked-prior-value" }
...
```
books/fix-your-gut-for-good/pipeline-state.json
  publishing.kdp_select → verify re-enrolment; term expired 2026-07-19 (now 19 days overdue)
  If re-enrolled: publishing.kdp_select_term_start = "2026-07-19", publishing.kdp_select_term_end = "2026-10-17"
  URGENT: Countdown Deal window ~2026-08-18 is 11 days away — re-enrolment must be confirmed now.
```

---

*Review velocity run: 2026-08-07 | Prior report: 2026-07-31 (7 days) | Fetch method: WebFetch (EGRESS_BLOCKED both Amazon.co.uk and Amazon.com) → WebSearch (no cached results) → all blocked | Gmail: 0 review-related emails | No counts invented | No pipeline-state.json files modified.*
