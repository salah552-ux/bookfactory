# Review Velocity Report — 2026-06-26

**Total new reviews this week (both books combined): UNVERIFIABLE — Amazon fetch blocked (9th+ consecutive block). Prior values used. Both books remain at 0 verified reviews. Review-generation action required on both titles — both are stalled at Pre-Gate 1 (ads locked).**
...
Amazon's bot-protection layer continues to block all automated fetch attempts. This is the **9th+ consecutive blocked run** (first block observed 2026-06-07; weekly velocity reports blocked since 2026-06-12). **No review counts have been invented.** Per hard rules, prior values (0) are used and flagged as unverified.

---

## Review Velocity Table

| Book | ASIN | Prior Count (2026-06-19) | Current Count | New This Period | Days Elapsed | Reviews / Week | Current Gate | To Next Gate | ETA to Next Gate |
|------|------|--------------------------|---------------|----------------|-------------|---------------|-------------|-------------|-----------------|
...
- **Note:** Pipeline-state.json still shows `kdp_status: "not_started"` and `asin: null` despite the book being treated as live per the manifest ASIN B0GXYLWS1W. State reconciliation has been outstanding since 2026-06-12 (14+ days unresolved). Not this agent's remit to fix.

### Death in the Cathedral Close (B0GZD1S8HF)

- **Current gate:** Pre-Gate 1 (0 / 5 reviews — unverified; fetch blocked)
- **Reviews to Gate 1:** 5
- **Days since live date:** 54 days (live since 2026-05-03)
- **Velocity:** 0 reviews/week (unverified; 54 days live, 0 verified reviews)
- **ETA to Gate 1:** STALLED — review-generation action needed
- **Note:** Pipeline-state.json exists at `books/untitled-cosy-mystery/pipeline-state.json` (the slug `death-in-the-cathedral-close` directory does not exist). This discrepancy has been flagged in prior reports. Countdown Deal window (2026-06-02 to 2026-06-09) has closed; whether it ran is unverified. `arc_emails_sent = 0` — ARC programme has not been activated.

---

## Review-Generation Actions Required (both books stalled at 0 for 54+ days)
...
- **This is the single highest-leverage action available. It should have been activated at launch (2026-05-03). Every week of delay is a week of unlockable ad spend lost.**
...
- Amazon.co.uk has blocked WebFetch and WebSearch for 9+ consecutive runs (since 2026-06-07 / 3 weeks)
...
books/fix-your-gut-for-good/pipeline-state.json
  post_launch.weekly_log → append:
    { "date": "2026-06-26", "review_count": 0, "avg_rating": null, "source": "fetch-blocked-prior-value" }
...
**Outstanding state hygiene (unresolved 14+ days — not this agent's remit):**
...
*Review velocity run: 2026-06-26 | Fetch method: WebFetch (×2 per ASIN, dp/ and product-reviews/ URLs, 403) → WebSearch fallback (no cached results) → both blocked | Gmail: 0 review-related emails | No counts invented | No pipeline-state.json files modified.*
