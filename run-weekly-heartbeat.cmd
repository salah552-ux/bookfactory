...
rem (two triggers: weekly Mon 09:00 + at logon; the stamp check below makes extra
rem fires harmless — it only runs if the last run is >6 days old).
cd /d C:\Users\salah\BookFactory
if not exist automation\reports mkdir automation\reports

rem Stamp check: skip if we ran within the last 6 days
set STAMP=automation\reports\.heartbeat-stamp
if exist %STAMP% (
  forfiles /p automation\reports /m .heartbeat-stamp /d -6 >nul 2>&1 || exit /b 0
)
echo ran > %STAMP%
