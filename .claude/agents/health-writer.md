...
After writing, the chapter must be passed to the book-reviewer agent before it is saved. A chapter is only approved for the manuscript folder if it scores 96/120 (grade B) or above. If it scores below 96, apply the top fixes and resubmit for review before saving.
